...
export default function UserInfoCard({
  ...
  return (
    <section className="mt-6 md:mt-8 bg-white rounded-lg shadow-md overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex flex-col lg:flex-row gap-4 md:gap-6">
            ...
            <Link href={`/profile/${userProfile.uid}`} className="flex-1">
              <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-lg bg-white hover:bg-gray-50 border border-transparent hover:border-[#2FA3E3] transition-colors cursor-pointer h-full">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs md:text-sm text-gray-500 truncate">@{userProfile.username}</p>
                  {userProfile.bio && (
                    <p className="text-xs md:text-sm text-gray-700 mt-1 md:mt-2 break-words whitespace-pre-wrap line-clamp-3">
                      {userProfile.bio}
                    </p>
                  )}
                </div>
            ...
            {(showRating || showActions) && (
              <div className="flex flex-col gap-3 md:gap-4 lg:w-80">
                {/* 評価カード */}
                {showRating && (
                  <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-200 shadow-sm flex-1 flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                      <div className="text-2xl md:text-3xl font-bold">{averageRating > 0 ? averageRating.toFixed(1) : '---'}</div>
