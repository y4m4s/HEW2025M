...
function PayCheckForm() {
  ...
  useEffect(() => {
    ...
    if (addressData) {
      ...
      setShippingFee(calculateShippingFee(address.prefecture));
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let orderId = '';
    try {
      // OrderItem形式に変換（productIdなど必要なフィールドを追加）
      const orderItems = await Promise.all(items.map(async (item) => {
        try {
          const response = await fetch(`/api/products/${item.id}`);
          if (!response.ok) {
            throw new Error(`商品ID ${item.id} の取得に失敗しました`);
          }
          ...
          console.error(`商品ID ${item.id} の詳細取得エラー:`, error);
          // エラー時はカートデータから基本情報のみ使用
          return {
            productId: item.id,
            productName: item.title,
            productImage: item.image || '',
    ...
    if (items.length && user) {
      fetch("/api/create-payment-intent", {
        ...
        body: JSON.stringify({
          items,
          userId: user.uid,
          amount: items.reduce((a, b) => a + b.price * b.quantity, 0) + 800,
